...
- Logs UI (`/logs`): per-user application log explorer. Server page scopes every query by the session user id via `src/domain/log/service.ts` (`listApplicationLogs`); client `log-explorer.tsx` provides level/event/request-id filters, UTC time range, pagination (25/page) and expandable rows with redacted metadata. Domain: `src/domain/log/` (schema, repository, service).
...
Checks (all green as of this writing): `npm run typecheck`, `npm run build` (13 routes including `/logs` and `/docs/apple-wallet-shortcut`), `npm run lint` (0 warnings), `npm test` (158 passed: money 19, crypto 7, keys 11, transaction schema 19, error mapper 7, logger 13, body-inspect 6, axiom-sink 11, db-sink 12, log service 9, route observability 8, api-key auth 6, plus existing notion/crypto tests).
