...
Checks (all green as of this writing): `npm run typecheck`, `npm run build` (13 routes including `/logs` and `/docs/apple-wallet-shortcut`), `npm run lint` (0 warnings), `npm test` (165 passed: money 19, crypto 7, keys 11, transaction schema 19, error mapper 7, logger 13, body-inspect 6, axiom-sink 11, db-sink 13, log service 12, route observability 11, api-key auth 6, plus existing notion/crypto tests).
